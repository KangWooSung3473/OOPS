...
const MyPage = () => {
  ...
  const { onUpdateUserInfo, onDeleteUserInfo } =
  ...
  const identifyDuplicated = async () => {
    ...
    try {
      ...
      setMsg(
        available ? "사용 가능한 아이디입니다." : "중복된 아이디가 존재합니다!",
      );
    } catch {
  ...
  const changeSetting = async () => {
    ...
    try {
      ...
      if (
        changeUserName.trim() !== "" &&
        changeUserName !== currentUserData.userName
      ) {
  ...
  const deleteAccount = async () => {
    if (
      window.confirm(
        "정말로 회원 탈퇴를 진행하시겠습니까? 지워진 계정은 복구되지 않습니다.",
      )
    ) {
      try {
      ...
      <main className="content-area max-width-wrapper">
        <div className="settings-card">
          ...
          <div className="settings-footer-row">
            <div
              className="settings-footer-row__left"
